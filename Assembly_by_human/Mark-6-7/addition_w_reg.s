...
CheckCircle:                                           # Формальные параметры: const double X[], const double Y[]
	endbr64
	push	rbp
	mov	rbp, rsp
	sub	rsp, 144
	...
	mov	rsi, rdx
	mov	rdi, rax
	call	CheckSimilarPoints                        # Аргументы double X[], double Y[] передаются через регистры rdi, rsi
...
.L2:
	...
	mov	rsi, rdx
	mov	rdi, rax
	call	FindCenter                                  # Аргументы double* &center_x, double* &center_y, double mx01, double my01, double dx01, double dy01, double mx12, double my12, double dx12, double dy12
	                                                    # передаются через регистры rdi, rsi, xmm0, xmm1, xmm2, xmm3, xmm4, xmm5, xmm6, xmm7
    test	eax, eax                                    # возвращаемое значение в eax
	jne	.L4
	mov	eax, 0
	jmp	.L9
...
.L9:
	leave
	ret
	.size	CheckCircle, .-CheckCircle                      # Возвращает код результата: 0 - не лежат на одной окружности, 1 - инчае
	.globl	CheckSimilarPoints
	.type	CheckSimilarPoints, @function
CheckSimilarPoints:                                     # Формальные параметры: const double X[], const double Y[]
	endbr64
	push	rbp
	mov	rbp, rsp
...
.L18:
	pop	rbp
	ret
	.size	CheckSimilarPoints, .-CheckSimilarPoints        # Возвращает код результата: 0 - все точки различные, 1 - есть одинаковые точки
	.globl	FindCenter
	.type	FindCenter, @function
FindCenter:                                             # Формальные параметры: double *center_x, double *center_y, double mx1, double my1, double ax, double ay, double mx2, double my2, double bx, double by
	endbr64
	push	rbp
	mov	rbp, rsp
...
.L27:
	pop	rbp
	ret
	.size	FindCenter, .-FindCenter                    # Возвращает код результата: 0 - прямые параллельные, 1 - найдена точка пересечения
	.section	.rodata
	.align 8
.LC5:
...
Output:                                                     # Формальные параметры: FILE *fin, double X[], double Y[], int belongs
	endbr64
	push	rbp
	mov	rbp, rsp
	sub	rsp, 32
	...
	mov	rdi, rax
	mov	eax, 8
	call	fprintf@PLT                                     # Аргументы FILE *fin, char* .LC5[rip], X[0], Y[0], X[1], Y[1], X[2], Y[2], X[3], Y[3] через регистры
	                                                        # rdi, rsi, xmm0, xmm1, xmm2, xmm3, xmm4, xmm5, xmm6, xmm7
...
.L32:
	...
	mov	rsi, rdx
	mov	rdi, rax
	call	fputs@PLT                                       # Аргументы FILE *fin, сhar* .LC7/.LC6 передаются через rdi, rsi
	nop                                                     # Возвращаемое значение в eax
	leave
	ret
	.size	Output, .-Output                                # Нет возвращаемого значения
	.section	.rodata
.LC8:
	.string	"%lf"
	.text
...
ReadFromFile:                                               # Формальные параметры: FILE *fin, double array_x[], double array_y[]
	endbr64
	push	rbp
	mov	rbp, rsp
	sub	rsp, 48
...
.L38:
	...
	mov	rdi, rax
	mov	eax, 0
	call	__isoc99_fscanf@PLT                             # Аргументы FILE *fin, char* .LC8, double* &array_x[i] передаются через регистры rdi, rsi, rdx
	cmp	eax, -1                                             # Возвращаемое значение в eax
	je	.L35
	mov	eax, r15d                                           # r15d - int i
	cdqe
	lea	rdx, 0[0+rax*8]
	...
	mov	rdi, rax
	mov	eax, 0
	call	__isoc99_fscanf@PLT                             # Аргументы FILE *fin, char* .LC8, double* &array_y[i] передаются через регистры rdi, rsi, rdx
	cmp	eax, -1                                             # Возвращаемое значение в eax
	jne	.L36
.L35:
	mov	eax, 2
...
.L37:
	leave
	ret
	.size	ReadFromFile, .-ReadFromFile                    # Возвращает код результата: 0 - все точки прочитаны корректно, 2 - иначе
	.section	.rodata
	.align 16
.LC0:
	.long	0
